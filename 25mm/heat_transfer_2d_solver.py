...
Options:
    steel: 'DUPLEX' | '30HN2MFA' | '38HMJ' | '32CrMoV12-28'
...
# 32CrMoV12-28 property library (Kelvin-based)
from props_32CrMoV1228 import props as PROPS_32CR  # type: ignore
...
def heat_transfer_2d_solver(
    *,
    ...
    # -------------------- geometry --------------------
    # NOTE: This solver has two built-in geometries:
    #   - default "35 mm article" profile (original code)
    #   - a conservative 25 mm profile keyed off steel tag "32CrMoV12-28"
    #
    # If you have a measured OD/thickness profile for the 25 mm tube, replace the
    # z_breaks/Rout_breaks arrays below.

    steel_tag = str(steel).upper()  # local tag for geometry selection
    ...
    if ("32CR" in steel_tag) or (steel_tag in {"32CRMOV12-28", "32CRMOV1228"}):
        ...
        orig_l0 = 0.216
        orig_lm = 2.934
        orig_li = np.array([0.216, 0.385, 0.535, 0.880, 2.081, 2.980], dtype=float)

        # 25 mm "new" l0 and lm
        bore_d = 0.025
        l0 = orig_l0 * (bore_d / 0.035)
        lm = 2.000

        frac = (orig_li - orig_l0) / max(orig_lm - orig_l0, EPS)
        z_mid = l0 + frac * (lm - l0)

                # --- Outer radius profile (measured 25 mm tube), piecewise in z_abs (breech-origin) ---
        # Confirmed geometry (OD in mm):
        #   0–125 mm: 76.5
        #   125–665 mm: 80
        #   665–771 mm: 86
        #   771–1915 mm: linear 86 → 38
        #   1915–2000 mm: 45
        #
        # Implemented as 5 sections using edges; the taper is handled explicitly below.
        z_edges = np.array([0.0, 0.125, 0.665, 0.771, 1.915, 2.000], dtype=float)

        OD0 = 76.5e-3
        OD1 = 80.0e-3
        OD2 = 86.0e-3
        OD_taper_end = 38.0e-3
        OD_muzzle = 45.0e-3

        def Rout_of_z(z_abs_m: np.ndarray) -> np.ndarray:
            """Outer radius [m] as a function of axial position z_abs [m]."""
            z = np.asarray(z_abs_m, dtype=float)
            Rout = np.empty_like(z)
            ...
            # 0.771–1.915 m: linear taper OD2 → OD_taper_end
            m3 = (z >= 0.771) & (z < 1.915)
            if np.any(m3):
                frac3 = (z[m3] - 0.771) / max(1.915 - 0.771, EPS)
                OD3 = OD2 + frac3 * (OD_taper_end - OD2)
                Rout[m3] = 0.5 * OD3

            # 1.915–2.000 m: muzzle cylinder
            m4 = z >= 1.915
            Rout[m4] = 0.5 * OD_muzzle

            return Rout

        # Outer radius profile along z:
        #   - "step":        piecewise-constant by zone (discontinuous shoulders)
        #   - "interp_edges":piecewise-linear between the z-edge stations (tapered look)
        #   - "smoothstep":  smoothed shoulders around z-edges using tanh blending
        outer_profile = "step"
        blend_w = 0.010  # [m], only used for "smoothstep"
        # Outer-radius values at geometry edges (used for optional interpolation/smoothing modes)
        Rout_edges = Rout_of_z(z_edges)

        # Representative radii per interval (legacy support)
        z_mids = 0.5 * (z_edges[:-1] + z_edges[1:])
        Rout_zone = Rout_of_z(z_mids)

        def _Rout_step(zq: np.ndarray) -> np.ndarray:
            return Rout_of_z(zq)

        def _Rout_interp_edges(zq: np.ndarray) -> np.ndarray:
            zq = np.asarray(zq, dtype=float)
            return np.interp(zq, z_edges, Rout_edges, left=Rout_edges[0], right=Rout_edges[-1])

        def _Rout_smoothstep(zq: np.ndarray) -> np.ndarray:
            return Rout_of_z(zq)

        sections = {
            "z_mid": z_mid.astype(float),
            "R_out": _Rout_interp_edges(z_mid).astype(float),
    ...
    def Rout_fun(zz: np.ndarray) -> np.ndarray:
        ...
        # 25 mm case: use the selected outer radius profile (see geometry block above)
        try:
            if outer_profile == "step":  # type: ignore[name-defined]
                return _Rout_step(zz)  # type: ignore[name-defined]
            if outer_profile == "smoothstep":  # type: ignore[name-defined]
                return _Rout_smoothstep(zz)  # type: ignore[name-defined]
            # default: "interp_edges"
            return _Rout_interp_edges(zz)  # type: ignore[name-defined]
        except NameError:
            pass

        # default: piecewise-linear interpolation with linear extrapolation
    ...
    # -------- build piecewise time vector (fine during hot-gas exposure, coarse otherwise) --------
    # IMPORTANT: In the 25 mm repeated-rifling case, we keep a nonzero "ambient" inner convection
    # coefficient (e.g. h_in=9.2) during long pauses. Therefore h(t)>0 does NOT mean "still heating".
    # We detect the end of hot-gas exposure primarily using Tg(t) relative to ambient.
    Tamb_C = float(T0)
    try:
        if (bc is not None) and isinstance(bc, dict):
            Tamb_C = float(bc.get("meta", {}).get("Tamb_C", Tamb_C))
    except Exception:
        pass

    Tg_thresh_C = Tamb_C + 1.0  # 1°C above ambient => still "hot gas"
    idx_hot = np.where(Tgas > Tg_thresh_C)[0]
    if idx_hot.size > 0:
        t_heat_end = float(t_ib[idx_hot[-1]])
    else:
        # fallback: if Tg never exceeds ambient (degenerate case), fall back to h threshold
        h_eps = 1e-6
        h_max_over_z = np.max(h_data, axis=1)
        idx_last_heat = np.where(h_max_over_z > h_eps)[0]
        t_heat_end = float(t_ib[idx_last_heat[-1]]) if idx_last_heat.size else 0.0

    # Tail end time can extend beyond BC end
    t_tail_end = t_heat_end + max(0.0, float(cool_tail_s))

    if user_set_tEnd:
        t_stop = float(tEnd_fd)
    else:
        # If bc is given and user didn't force tEnd, default stop time is at least BC end
        t_stop = float(max(tEnd_fd, t_tail_end))

    # 1) Fine dt during hot-gas exposure
    t = _arange_inclusive(0.0, float(dt_fd), min(t_heat_end, t_stop))

    # 2) Coarse dt for everything after hot-gas exposure (including long pauses and tail cooling)
    if t_stop > t[-1] + 1e-12:
        t2 = _arange_inclusive(t[-1] + float(dt_tail), float(dt_tail), t_stop)
    ...
    # Default behavior keeps the original tabulated steels; 32CrMoV12-28 uses props_32CrMoV1228.py (Kelvin-based).

    if ("32CR" in steel) or (steel in {"32CRMOV12-28", "32CRMOV1228"}):
        if PROPS_32CR is None:
            raise ImportError("steel='32CrMoV12-28' requires props_32CrMoV1228.py on your PYTHONPATH.")
        # props_32CrMoV1228 expects Kelvin; solver state uses °C
        def kS_fun(Tc): return PROPS_32CR.steel.k(np.asarray(Tc, dtype=float) + 273.15)
        def cpS_fun(Tc): return PROPS_32CR.steel.cp(np.asarray(Tc, dtype=float) + 273.15)
        def rhoS_fun(Tc): return PROPS_32CR.steel.rho(np.asarray(Tc, dtype=float) + 273.15)

        def kC_fun(Tc): return PROPS_32CR.cr.k(np.asarray(Tc, dtype=float) + 273.15)
        def cpC_fun(Tc): return PROPS_32CR.cr.cp(np.asarray(Tc, dtype=float) + 273.15)
        def rhoC_fun(Tc): return PROPS_32CR.cr.rho(np.asarray(Tc, dtype=float) + 273.15)
    ...
    T_inner_probe = T0 * np.ones((Nt, int(probe_j.size)), dtype=float)
    T_outer_probe = T0 * np.ones((Nt, int(probe_j.size)), dtype=float)
    ...
    for n in range(Nt - 1):
        ...
        # log extra axial probes (inner & outer cells at specified x)
        for pk in range(int(probe_j.size)):
            jj = int(probe_j[pk])
            T_inner_probe[n + 1, pk] = T[0, jj]
            ii = int(i_end[jj])
            if ii > 0:
                T_outer_probe[n + 1, pk] = T[ii - 1, jj]
            else:
                T_outer_probe[n + 1, pk] = np.nan
    ...
    if Nt >= 2:
        ...
        T_inner_probe[-1, :] = T_inner_probe[-2, :]
        T_outer_probe[-1, :] = T_outer_probe[-2, :]
    ...
    if plot:
        ...
        # axial probe traces (requested): x≈199 mm, x≈400 mm, muzzle tip
        figp1, axp1 = plt.subplots(figsize=(9, 4))
        figp1.canvas.manager.set_window_title("Inner-surface T (axial probes)")
        axp1.plot(1e3 * t, T_inner_probe, linewidth=1.4)
        axp1.grid(True)
        axp1.set_xlabel("time [ms]")
        axp1.set_ylabel("Inner-surface T [°C]")
        axp1.set_title("Inner-surface temperature at axial probes")
        axp1.legend(probe_labels, loc="best")

        figp2, axp2 = plt.subplots(figsize=(9, 4))
        figp2.canvas.manager.set_window_title("Outer-surface T (axial probes)")
        axp2.plot(1e3 * t, T_outer_probe, linewidth=1.4)
        axp2.grid(True)
        axp2.set_xlabel("time [ms]")
        axp2.set_ylabel("Outer-surface T [°C]")
        axp2.set_title("Outer-surface temperature at axial probes")
        axp2.legend(probe_labels, loc="best")
    ...
    return {
        ...
        "T_inner6": T_inner6,
        "T_outer6": T_outer6,
        "T_inner_probe": T_inner_probe,
        "T_outer_probe": T_outer_probe,
        "probe": {"x_targets_m": probe_x_m, "j_idx": probe_j, "z_nearest_m": probe_z_m, "labels": probe_labels},
        "Rout_z": Rout_z,
        "sections": sections,
        "opts": {
